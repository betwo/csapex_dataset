...
void PeopleDatasetImporter::process()
{
    if (play_index_ < play_entries_.size())
    {
        ...
        {
            CvMatMessage::Ptr depth_msg = std::make_shared<CvMatMessage>(enc::depth, "unknown", entry.timestamp);
            CvMatMessage::Ptr visual_msg = std::make_shared<CvMatMessage>(enc::bgr, "unknown", entry.timestamp);
